...
function taskDate(task) { return task.deadline || task.createdAt?.slice(0, 10) || '' }
...
export default function TimesheetPage() {
  ...
  const rows = useMemo(() => {
    ...
    const adjustmentRows = adjustments.map((item) => {
      ...
      return {
        ...
        date: item.date, amount: item.type === 'addition' ? item.amount : -item.amount, adjustment: item,
      }
    })
  ...
  const save = async (event) => {
    ...
    if (!form.userId || Number(form.amount) <= 0 || !form.description.trim() || !form.date || saving) return
    setSaving(true)
    try {
      const profile = profilesById.get(form.userId)
      const payload = { ...form, userName: nameOf(profile), amount: Number(form.amount) }
  ...
  return <>
    ...
    <div className="timesheet-page">
      <div className="timesheet-summary-grid">{[['Tổng tiền', money(summary.total)], ['Việc hoàn thành', summary.tasks], ['Số người', summary.users], ['Số dự án', summary.projects]].map(([label, value]) => <article className="timesheet-summary-card" key={label}><p>{label}</p><strong>{value}</strong></article>)}</div>
      <Card><div className="timesheet-filters"><Filter label="Từ ngày"><input className="input" type="date" value={filters.from} onChange={(e) => setFilters({ ...filters, from: e.target.value })} /></Filter><Filter label="Đến ngày"><input className="input" type="date" value={filters.to} onChange={(e) => setFilters({ ...filters, to: e.target.value })} /></Filter><Filter label="Dự án"><select className="select" value={filters.projectId} onChange={(e) => setFilters({ ...filters, projectId: e.target.value })}><option value="all">Tất cả</option>{projects.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Filter><Filter label="Người thực hiện"><select className="select" value={filters.userId} onChange={(e) => setFilters({ ...filters, userId: e.target.value })}><option value="all">Tất cả</option>{profiles.map((item) => <option key={item.id} value={item.id}>{nameOf(item)}</option>)}</select></Filter><Filter label="Bộ phận"><select className="select" value={filters.departmentId} onChange={(e) => setFilters({ ...filters, departmentId: e.target.value })}><option value="all">Tất cả</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Filter><Filter label="Loại"><select className="select" value={filters.kind} onChange={(e) => setFilters({ ...filters, kind: e.target.value })}><option value="all">Tất cả</option><option value="task">Công việc</option><option value="addition">Cộng thêm</option><option value="deduction">Khấu trừ</option></select></Filter></div></Card>
...
function summarize(rows, key, nameKey) { const map = new Map(); rows.forEach((row) => { const item = map.get(row[key]) || { id: row[key], name: row[nameKey], tasks: 0, additions: 0, deductions: 0, total: 0 }; if (row.kind === 'task') item.tasks += 1; if (row.kind === 'addition') item.additions += row.amount; if (row.kind === 'deduction') item.deductions += Math.abs(row.amount); item.total += row.amount; map.set(row[key], item) }); return [...map.values()].sort((a, b) => b.total - a.total).map((item) => ({ ...item, value: item.total, label: item.name })) }
...
function SummaryTable({ title, items, people = false }) { return <Card title={title}><div className="timesheet-table-wrap"><table className="timesheet-table"><thead><tr><th>{people ? 'Người' : 'Dự án'}</th><th>Công việc</th>{people && <><th>Cộng thêm</th><th>Khấu trừ</th></>}<th>Tổng</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td>{item.name}</td><td>{item.tasks}</td>{people && <><td>{money(item.additions)}</td><td>{money(item.deductions)}</td></>}<td>{money(item.total)}</td></tr>)}</tbody></table></div></Card> }
